...
public class TelegramBot extends TelegramLongPollingBot {
    ...
    public TelegramBot(PersonFormDispatcher personFormDispatcher, MessageInfoService messageInfoService,
                       SearchVacancyDispatcher searchVacancyDispatcher, CallbackVacancyDispatcher callbackVacancyDispatcher, WebClient.Builder webClientBuilder, GatewayMainDispatcher gatewayMainDispatcher) {
    ...
    public void onUpdateReceived(Update update) {
        ...
        BotResponse botResponse = gatewayMainDispatcher.dispatch(update);
...
// метод обработки ответа от микросервисов
    private void executeCommand(BotCommand command) {
        try {
            switch (command.type()) {
                case SEND_MESSAGE -> execute(
                        new SendMessage(command.chatId().toString(), command.text())
                );
